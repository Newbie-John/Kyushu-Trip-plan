...
$finalContent = $cleanTop + "," + "`r`n" + $daysStr + "`r`n" + $goshuinStr

# Because flights and wallet are already correct from previous run, we just overwrite the rest.
# Wait, let's make sure the wallet content isn't duplicated or lost.
# Ah, $cleanTop includes ONLY Day 1 to Day 6. It DOES NOT include the wallet block because I stripped it out at $closingBrace + $match.Length.
# My bad! I need to re-insert the wallet block!
...
$finalContent = $cleanTop + "," + "`r`n" + $daysStr + "`r`n  " + $walletStr + "`r`n" + $goshuinStr

$utf8NoBom = New-Object System.Text.UTF8Encoding $False
[System.IO.File]::WriteAllText($dataJsPath, $finalContent, $utf8NoBom)
Write-Output "Patch V6 Successful"
